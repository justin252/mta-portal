...
**Deploy**: `cp boot.py code.py logic.py *.bmp /Volumes/CIRCUITPY/ && cp -r fonts /Volumes/CIRCUITPY/`. Train bitmap (`train.bmp`) is generated by `generate_train_bitmap.py`. Board auto-runs `code.py` on save (auto-reload).
...
- Button toggle: short press UP → L train, short press DOWN → G train. Long press (1s) either → toggle full-screen train pixel art view. Short press in train view → return to arrivals. Defaults to L on boot.
